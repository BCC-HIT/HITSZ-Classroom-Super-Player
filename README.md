...
*  **🎯 8秒实时自动对齐 (FFT Cross-Correlation)**：点击控制栏上的 🎯 按钮，脚本会自动异步捕获双流 8 秒的实时音频波形，在后台通过高精度的 **FFT 互相关算法** 瞬间计算出时间轴偏移量并自动对齐（感知时间极其短暂），彻底解决老师画面与 PPT 声画不同步的痛点。
...
点击视角对齐旁的 **🎯** 按钮可以一键校准，但为了保证算法 100% 成功，请遵循以下前提： 

1. **先粗调，后精调**：请先手动调节滑块，将主副画面的偏差缩小到 **5秒以内**。 
2. **选择连贯说话片段**：请在**老师正在持续、连贯讲话**的片段点击按钮。如果是大段的课间安静、播放眼保健操或纯放音乐，由于没有有效的人声特征波形，可能会导致对齐失
